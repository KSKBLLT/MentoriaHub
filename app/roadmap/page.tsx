        ...
        {steps.map((s, i) => (
          <div className="card col" key={s.id} style={firstTodoId === s.id ? { borderColor: "var(--accent)", borderWidth: 2 } : undefined}>
            ...
            <div className="row" style={{ gap: 16 }}>
              <div className="elig-qualify">Если прошёл: <span style={{ color: "var(--muted)", fontWeight: 400 }}>{s.onPass}</span></div>
            </div>
            <div className="elig-soon">Если нет: <span style={{ color: "var(--muted)", fontWeight: 400 }}>{s.onFail}</span></div>
            {s.status === "todo" && (
              <div className="row">
